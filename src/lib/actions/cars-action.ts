...
// bookmark car action
// This function toggles the bookmark status of a car for the authenticated user
export const bookmarkCar = async (carId: string) => {
  ...
  if (isAlreadySaved) {
    ...
    await prisma.car.update({
      ...
      data: {
        savedBy: {
          // this is a prisma feature that allows you to update relations
          // disconnect removes the relation between the car and the user
          // connect(a function by prisma) is used to add a new relation between both the car and the user it updates both the car model and the user model
          connect: {
            id: user.id,
          },
        },
  ...
  // this is used to revalidate i.e to refresh the page data and show the updated state of the car
  revalidatePath(`/cars/${carId}`);
};
