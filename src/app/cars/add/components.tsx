...
import { useImages } from "@/app/store/image-store";
import {
  Image,
  ImageKitAbortError,
...
import { imageKitAuthenticator } from "@/lib/actions/image-kit";
...
export const AddCarForm = () => {
  ...
  const {
    ...
    // @ts-expect-error // error due to the use of react-hook-form with zod
  ...
  const autoGenerateHandler = useCallback(async () => {
    try {
      ...
      // Generate Info
      const result = await autoGenerateCar(watch("name") as string);
      ...
      Object.entries(result).forEach(([key, value]) => {
        if (key === "images") {
          console.log("fsfsfdfdsfsfsdfdsfsdfsf");
          return;
        }
  ...
  useEffect(() => {
    ...
    // getting ai genearated car details from localStorage and set to form state after reloading
